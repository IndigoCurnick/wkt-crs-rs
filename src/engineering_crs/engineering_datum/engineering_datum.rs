use crate::{
    ...
    datum::DatumAnchor,
    error::WktParseError,
    keywords::{ANCHOR, EDATUM, ENGINEERINGDATUM, ID},
...
impl TryFrom<&WktNode> for EngineeringDatum {
    type Error = WktParseError;

    fn try_from(value: &WktNode) -> Result<Self, Self::Error> {
        if !(value.keyword == EDATUM || value.keyword == ENGINEERINGDATUM) {
            return Err(WktParseError::IncorrectKeyword {
                expected: vec![EDATUM.into(), ENGINEERINGDATUM.into()].into(),
