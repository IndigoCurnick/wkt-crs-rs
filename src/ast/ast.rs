...
impl Parse<f64> for WktArg {
	fn parse(&self) -> Result<f64, WktParseError> {
		let num_str = match self {
			Self::Data(s) => s,
			Self::Node(_) => {
				return Err(WktParseError::ExpectedNumber {
					arg: self.clone(),
				});
			}
		};
		...
		return match num_str.parse::<f64>() {
			Ok(x) => Ok(x),
			Err(_) => Err(WktParseError::ExpectedNumber { arg: self.clone() }),
		};
	}
}
...
impl Parse<NumText> for WktArg {
	fn parse(&self) -> Result<NumText, WktParseError> {
		return match self {
			...
			Self::Node(_) => {
				Err(WktParseError::ExpectedStringOrNumber { arg: self.clone() })
			}
		};
	}
}
...
impl Parse<DateOrString> for WktArg {
	fn parse(&self) -> Result<DateOrString, WktParseError> {
		let date_str = match self {
			Self::Data(s) => s,
			Self::Node(_) => {
				return Err(WktParseError::ExpectedStringOrDate {
					arg: self.clone(),
				});
			}
		};
