...
impl WktBaseType for CoordinateSystem {
        ...
        I: IntoIterator<Item = &'a WktNode>,
    {
        ...
        if let Ok(ordinal) = OrdinalDateTimeCoordinateSystem::from_nodes(iter.clone()) {
        ...
        if let Ok(temporal) = TemporalCountMeasureCoordinateSystem::from_nodes(iter) {
            return Ok(WktBaseTypeResult {
                result: CoordinateSystem::TemporalCountMeasureCS(temporal.result),
...
impl WktInlineType for CoordinateSystem {
        ...
        I: IntoIterator<Item = &'a crate::ast::WktArg>,
    {
        ...
        if let Ok(ordinal) = OrdinalDateTimeCoordinateSystem::from_args(iter.clone()) {
        ...
        if let Ok(temporal) = TemporalCountMeasureCoordinateSystem::from_args(iter) {
            return Ok(WktInlineResult {
                result: CoordinateSystem::TemporalCountMeasureCS(temporal.result),
                consumed: temporal.consumed,
            });
