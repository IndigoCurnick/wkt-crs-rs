use crate::{
    ast::{Parse, WktArg},
    base_types::{AreaDescription, GeographicBoundingBox, TemporalExtent, VerticalExtent},
    error::WktParseError,
    keywords::Keywords,
...
impl WktInlineType for Extent {
    fn from_args<'a, I>(wkt_nodes: I) -> Result<WktInlineResult<Self>, WktParseError>
    where
        ...
        while let Some(arg) = it.next() {
            match arg {
                WktArg::Node(node) => match node.keyword {
                    Keywords::Area => {
                        if area.is_some() {
                            return Err(WktParseError::TooManyKeyword(Keywords::Area));
                        }
                        i += 1;
                    ...
                    Keywords::BBox => {
                        if geo.is_some() {
                            return Err(WktParseError::TooManyKeyword(Keywords::BBox));
                        }
                        i += 1;
                        geo = Some(node.parse()?);
                    }
                    Keywords::VerticalExtent => {
                        if vert.is_some() {
                            return Err(WktParseError::TooManyKeyword(Keywords::VerticalExtent));
                        }
                        i += 1;
                    ...
                    Keywords::TimeExtent => {
                        if temp.is_some() {
                            return Err(WktParseError::TooManyKeyword(Keywords::TimeExtent));
                        }
                        i += 1;
