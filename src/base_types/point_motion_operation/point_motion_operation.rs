use crate::{
    ...
    base_types::{OperationAccuracy, OperationMethod, OperationVersion, SourceCrs},
...
impl WktBaseType for PointMotionOperation {
        ...
        I: IntoIterator<Item = &'a WktNode>,
    {
        ...
        match_keywords(&node.keyword, vec![Keywords::PointMotionOperation])?;
        lower_bound_arity(node.args.len(), 4)?;

        let mut i = 0;
        let len = node.args.len();

        let operation_name = node.args[i].parse()?;
        ...
        let compound = PointMotionOperation {
            operation_name,
            operation_version,
            source_crs,
            operation_method,
