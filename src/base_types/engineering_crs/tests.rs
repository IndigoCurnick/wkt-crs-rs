use crate::{
	ast::parse_wkt,
	base_types::{
		Axis, CoordinateSystem, DatumAnchor, EngineeringDatum, Extent, Id,
		LengthUnit, Order, OrdinalDateTimeCoordinateSystem, Scope,
...
fn test_example_4() {
	let correct = EngineeringCrs {
		...
		engineering_datum: EngineeringDatum {
			...
			datum_anchor: Some(DatumAnchor(
				"Top left corner of image = 0,0".into(),
			)),
			identifier: None,
		},
		coordinate_system: CoordinateSystem::SpatialCS(
