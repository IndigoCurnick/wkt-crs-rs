use crate::{
    ...
    error::WktParseError,
    keywords::{ANGLEUNIT, BASEGEODCRS, DATUM, ENSEMBLE, GEODETICDATUM, ID, TRF, UNIT},
...
pub struct BaseDynamicGeodeticCrs {
...
impl TryFrom<&WktNode> for BaseDynamicGeodeticCrs {
    type Error = WktParseError;

    fn try_from(value: &WktNode) -> Result<Self, Self::Error> {
        if value.keyword != BASEGEODCRS {
            return Err(WktParseError::IncorrectKeyword {
                expected: vec![BASEGEODCRS.into()].into(),
        ...
        return Ok(BaseDynamicGeodeticCrs {
            base_crs_name,
            dynamic_crs,
            geodetic_data,
            ellipsoidal_cs_unit,
