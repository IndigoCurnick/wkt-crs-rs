...
impl WktBaseType for SpatialUnit {
		...
		I: IntoIterator<Item = &'a WktNode>,
	{
		...
		let unit = match &node.keyword {
			Keywords::LengthUnit => SpatialUnit::LengthUnit(
				LengthUnit::from_nodes(vec![node])?.result,
			),
			Keywords::AngleUnit => SpatialUnit::AngleUnit(
				AngleUnit::from_nodes(vec![node])?.result,
			),
			Keywords::ScaleUnit => SpatialUnit::ScaleUnit(
				ScaleUnit::from_nodes(vec![node])?.result,
			),
			Keywords::ParametricUnit => SpatialUnit::ParametricUnit(
				ParametricUnit::from_nodes(vec![node])?.result,
			),
			Keywords::Unit => {
				error!(
