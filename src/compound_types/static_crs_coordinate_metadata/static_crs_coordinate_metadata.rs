use crate::{
	ast::{WktArg, WktNode},
	base_types::{
		CompoundCrs, DerivedEngineeringCrs, DerivedGeodeticCrs,
		DerivedParametricCrs, DerivedProjectedCrs, DerivedTemporalCrs,
		DerivedVerticalCrs, EngineeringCrs, GeodeticCrs, ParametricCrs,
...
impl WktBaseType for StaticCrsCoordinateMetadata {
		...
		I: IntoIterator<Item = &'a WktNode>,
	{
		...
		let first_keyword = if let Some(nod) = iter.get(0) {
			...
			// Just some default, if there's no nodes I guess?
