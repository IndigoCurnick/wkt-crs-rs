...
impl WktBaseType for MapProjectionParameterUnit {
		...
		I: IntoIterator<Item = &'a WktNode>,
	{
		...
		let unit = match &node.keyword {
			Keywords::LengthUnit => MapProjectionParameterUnit::LengthUnit(
				LengthUnit::from_nodes(vec![node])?.result,
			),
			Keywords::AngleUnit => MapProjectionParameterUnit::AngleUnit(
				AngleUnit::from_nodes(vec![node])?.result,
			),
			Keywords::ScaleUnit => MapProjectionParameterUnit::ScaleUnit(
