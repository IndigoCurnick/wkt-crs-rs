use crate::{
    ast::parse_wkt, datum::GeodeticReferenceFrameDatum, ellipsoid::Ellipsoid,
    geodetic_crs::GeodeticData, scope_extent_identifier_remark::Id, units::LengthUnit,
    utils::NumText,
};
...
fn test_base_static_geographic_crs() {
    ...
    assert_eq!(ast.len(), 1);
    let map_proj = BaseStaticGeographicCrs::try_from(&ast[0]).unwrap();
