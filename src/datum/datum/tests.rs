...
use super::GeodeticReferenceFrameDatum;
...
fn test_datum() {
    // Example 1
    let correct = GeodeticReferenceFrameDatum {
    ...
    let datum = GeodeticReferenceFrameDatum::try_from((&ast[0], None)).unwrap();

    assert_eq!(correct, datum);
    ...
    let datum = GeodeticReferenceFrameDatum::try_from((&ast[0], Some(pm))).unwrap();
